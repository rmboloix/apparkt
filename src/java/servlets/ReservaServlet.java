...
public class ReservaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        }else if(request.getParameter("acc").equals("anular")){
            try {
                //Proceso de anulació de reserva per part del client.
                int id = Integer.parseInt(request.getParameter("idReserva"));
                db.iniciar();
                db.obrir();
                db.anularReserva(id);
                db.tancar();
                //Actualitzem tornem a la vista del client.
                RequestDispatcher disp = request.getRequestDispatcher("/client/index.jsp");
                disp.forward(request, response);
            } catch (UtilitatPersistenciaException ex) {
                System.err.println("Error intentant anular la reserva."+ex);
